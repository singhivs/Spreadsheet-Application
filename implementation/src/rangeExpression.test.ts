...
describe('Spreadsheet', () => {
  it('should update a range expression when adding a row', () => {
    ...
    const cells = [
      [new Cell(new StringLiteral('A1'), 0, 0), new Cell(new StringLiteral('B1'), 0, 1)],
      [new Cell(new StringLiteral('A2'), 1, 0), new Cell(new StringLiteral(''), 1, 1)],
    ];
    const maxRow = 5;
    const maxCol = 5;
    ...
    expect(spreadsheet.getCell(1,1).getCellContent().getContent()).to.equal('0');
    ...
    expect(spreadsheet.getCell(2,1).getCellContent().getContent()).to.equal('0');
  });
