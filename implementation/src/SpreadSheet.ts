...
export class Spreadsheet {
     ...
     */
    public constructor(maxRow: number, maxCol: number) {
        this.maxRow = maxRow;
        this.maxCol = maxCol;
    ...
    public addRow(index: number): void {
        ...
        const newRow: Cell[] = new Array(this.maxCol).fill(null).map((_, colIndex) => {
  ...
  public find(searchString: string): Array<[number, number]> {
    ...
    private updateRangeExpressionsForRowChange(rowIndex: number, changeType: 'add' | 'delete'): void {
        this.cells.forEach((row, i) => {
            row.forEach((cell, j) => {
                if (cell.getCellContent() instanceof RangeExpression) {
                    let rangeExpression = cell.getCellContent() as RangeExpression;
                    let updatedExpression = this.updateRangeExpression(rangeExpression.getContent(), rowIndex, null, changeType);
    ...
    private updateRangeExpressionsForColumnChange(columnIndex: number, changeType: 'add' | 'delete'): void {
        this.cells.forEach((row, i) => {
            row.forEach((cell, j) => {
                if (cell.getCellContent() instanceof RangeExpression) {
                    let rangeExpression = cell.getCellContent() as RangeExpression;
                    let updatedExpression = this.updateRangeExpression(rangeExpression.getContent(), null, columnIndex, changeType);
                    rangeExpression.setContent(updatedExpression);
                }
            });
        });
    ...
    private updateRangeExpression(expression: string, rowIndex: number | null, columnIndex: number | null, changeType: 'add' | 'delete'): string {
        ...
        return `${match[1].slice(0, -startRow.length)}${startCol}${startRow}:${endCol}${endRow}`;
    }
