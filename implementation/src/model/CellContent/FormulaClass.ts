...
export class Formula implements CellContent {
  ...
  private computeRangeExpressionsAndCellReferences(): string {
    ...
    computedVal = computedVal.replace(rangeExpressionRegex, (match) => {
      ...
      if (rangeExpression.getContent().localeCompare("") == 0) {
        return "0";
      }
    ...
    computedVal = computedVal.replace(cellReferenceRegex, (match) => {
      ...
      if (cellReference.getContent().localeCompare("") == 0) {
        return "0";
      }
  ...
  /**
   * Function to compute and replace string concatenation in the formula
   * string.
   */
  private computeStringConcatenation(computedVal: string): string {
    // regular expression to match string concatenation (e.g., "a" + "b")
    const stringConcatenationRegex = /"([^"]+)"\s*\+\s*"([^"]+)"/g;
  ...
  parse(): void {
    ...
    computedVal = this.computeStringConcatenation(computedVal);
