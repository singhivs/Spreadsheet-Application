...
describe("RangeExpression", () => {
  ...
  it("should throw an error for an invalid cell reference", () => {
    ...
    expect(() => new RangeExpression(invalidRange, spreadsheet).getContent()).to.throw(
      "Invalid cell reference."
    );
  });
  ...
  it("should handle an unsupported operation", () => {
    ...
    const rangeExpression = new RangeExpression(unsupportedOperation, spreadsheet);

    expect(() => rangeExpression.getContent()).to.throw("Unsupported operation.");
  });
